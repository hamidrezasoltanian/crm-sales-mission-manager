...
export default function KpiPage() {
  ...
  useEffect(() => {
    if (!authLoading && user && typeof window !== 'undefined') {
      ...
      if (!token) {
        console.error('[KPI] No token found')
        setLoading(false)
        return
      }
      ...
      <ProtectedRoute allowedRoles={['manager', 'admin', 'super_admin']}>
        <div className="fixed inset-0 bg-white flex items-center justify-center z-50">
          <div className="text-center">
            ...
            <p className="text-gray-600">در حال بارگذاری داشبورد KPI...</p>
          </div>
        </div>
      </ProtectedRoute>
    )
  ...
  // This makes it look exactly like the original dashboard at port 5000
  return (
    <ProtectedRoute allowedRoles={['manager', 'admin', 'super_admin']}>
      <div 
        style={{ 
      ...
      >
        {proxyUrl ? (
          <iframe
            ...
            onError={(e) => {
              console.error('[KPI] Iframe error:', e);
            }}
          />
        ) : (
