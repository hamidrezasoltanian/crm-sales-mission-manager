...
setInterval(() => {
  if (bot.isPolling && typeof bot.isPolling === 'function') {
    const status = bot.isPolling();
    console.log(`📊 [${new Date().toISOString()}] Polling status: ${status ? '✅ Active' : '❌ Inactive'}`);
...
bot.getMe().then((botInfo) => {
  ...
  console.log(`   🆔 ${botInfo.id}`);
  console.log(`\n⏳ در حال انتظار برای پیام /start...`);
  console.log('   لطفاً در تلگرام /start بزنید\n');
  console.log('   این اسکریپت هر 5 ثانیه وضعیت polling را چک می‌کند\n');
}).catch((error) => {
